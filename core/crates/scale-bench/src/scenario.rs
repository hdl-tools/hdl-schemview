...
//! at sizes it can afford to repeat. This module covers what criterion cannot:
//!
...
//! `match`.
//!
//! The logic lives here rather than in `src/bin/scenario.rs` (#240) because a
//! `[[bin]]` target cannot be imported: on an isolated machine there is no
//! separate `scenario` binary to spawn, only the packaged app running its hidden
//! `--bench-scenario` arm. The bin is now a shim over [`main_with`], so the two
//! entry points cannot drift apart. [`run`] **returns** the JSON line instead of
//! printing it, which is what lets [`crate::collect`] treat a scenario as a
//! value; only [`main_with`] writes to stdout.

use std::borrow::Cow;
use std::path::PathBuf;
use std::time::Instant;

use crate::{derive_handles, mem, signals_from_design, RealHandles};
use svxprobe_matcher::{run_match, MatchOptions};
use svxprobe_model::{Design, Dir, NodeId, NodeKind};

const CONE_DEPTH: usize = 6;

/// Navigation defaults. Named constants rather than literals because
/// `docs/benchmarking.md` states a hand-run with other values is not comparable
/// to a collected one — the collector relies on these defaults instead of
/// passing them, so this is the single place the two agree.
pub const NAV_SCOPES: usize = 32;
pub const NAV_ITERS: usize = 3;

// --- argument parsing -------------------------------------------------------

/// One scenario invocation's arguments.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ScenarioArgs {
    pub basis: String,
    pub mode: String,
    pub signals: Option<usize>,
    pub nav_scopes: usize,
    pub nav_iters: usize,
}

/// Parse a scenario command line (the iterator must exclude the program name).
///
/// Takes an iterator rather than reading `std::env::args()` so the app's
/// `--bench-scenario` arm can feed it a forwarded tail — the flags, the error
/// strings and the exit codes are then identical on both paths by construction.
pub fn parse_args(argv: impl IntoIterator<Item = String>) -> Result<ScenarioArgs, String> {
    let mut basis = None;
    let mut mode = None;
    let mut signals = None;
    let mut nav_scopes = NAV_SCOPES;
    let mut nav_iters = NAV_ITERS;
...
/// A flat key/value record rendered as one JSON object. Flat on purpose: the
/// collector pastes these into a markdown table without walking a tree.
///
/// Hand-rolled rather than `serde_json` because the *rendering* is the contract:
/// `num` always emits 4 decimals and `int` a bare integer, and the collector's
/// number formatting keys off exactly that float-vs-integer split when it reads
/// the line back.
#[derive(Default)]
struct Record {
...
impl Record {
    ...
    fn num(&mut self, key: &str, value: f64) -> &mut Self {
        // JSON has no NaN/Infinity: emit null rather than a document the
...
fn finish(mut rec: Record, args: &ScenarioArgs, wall_s: f64) -> String {
...
pub fn workdir(basis: &str) -> PathBuf {
    std::env::temp_dir()
...
#[cfg(feature = "synth")]
fn synth_config(basis: &str) -> Option<crate::SynthConfig> {
    match basis {
        "665" => Some(crate::SynthConfig::baseline()),
        "100K" => Some(crate::SynthConfig::mid()),
        "1M" => Some(crate::SynthConfig::large()),
        _ => None,
    }
}
...
///
/// `Cow` so the embedded golden (2.1 MB) is passed by reference — it already
/// sits in the binary's read-only data, and copying it would add to the
/// measurement's own footprint for no reason.
fn source_bytes(basis: &str) -> Result<Cow<'static, [u8]>, String> {
    #[cfg(feature = "synth")]
    if let Some(cfg) = synth_config(basis) {
        ...
        return Ok(Cow::Owned(crate::to_json(&crate::generate(&cfg).doc)));
    }
    match basis {
        "golden" => crate::golden::golden_bytes().ok_or_else(|| {
            "basis 'golden' needs the `golden` cargo feature (the fixture is embedded at \
             compile time)"
                .to_string()
        }),
        "real" => {
            let src = std::env::var_os("SCALE_BENCH_MODEL")
                .map(PathBuf::from)
...
fn run_prepare(args: &ScenarioArgs) -> Result<String, String> {
...
fn run_from_slice(args: &ScenarioArgs) -> Result<String, String> {
...
fn run_cache_hit(args: &ScenarioArgs) -> Result<String, String> {
...
fn run_access(args: &ScenarioArgs, checked: bool) -> Result<String, String> {
...
fn run_nav(args: &ScenarioArgs) -> Result<String, String> {
    let (model, _) = prepared_bytes(&args.basis)?;
...
fn run_match_mode(args: &ScenarioArgs) -> Result<String, String> {
    let (model, _) = prepared_bytes(&args.basis)?;
...
/// Usage text — the canonical scenario argument contract.
pub const USAGE: &str = "usage: scenario --basis <665|100K|1M|golden|real> \
--mode <prepare|from_slice|cache_hit|access_checked|access_unchecked|nav|match> \
[--signals N] [--nav-scopes N] [--nav-iters N]";

/// The load/memory modes, in the order a basis runs them. `prepare` is first
/// because every later mode reads what it materialized.
pub const LOAD_MODES: [&str; 5] = [
    "prepare",
    "from_slice",
    "cache_hit",
    "access_checked",
...
/// Run one scenario and **return** its JSON line rather than printing it, so a
/// caller can collect scenarios as values. Progress still goes to stderr.
pub fn run(args: &ScenarioArgs) -> Result<String, String> {
...
/// The scenario CLI contract, verbatim: one JSON line on stdout, progress and
/// errors on stderr, exit 2 on a bad command line and 1 on a failed run.
///
/// Both `src/bin/scenario.rs` and the app's `--bench-scenario` arm are thin
/// wrappers over this, so neither can drift from the documented contract.
pub fn main_with(argv: impl IntoIterator<Item = String>) -> i32 {
    ...
    match run(&args) {
        ...
        Err(e) => {
            // Stderr + a non-zero exit, so the collector records the failure as
            // a row instead of silently dropping the scenario. A 1M OOM *is* a
            // result.
