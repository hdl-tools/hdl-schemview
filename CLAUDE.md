...
| `trace_graph` | `steps: [{path, dir, depth?, fanout?}]`, `limits?`, `projection?` | `SchematicGraph` (#244 PR2) — the accumulated fan-in/out of an ordered step list. Seeds are canonical **paths** (what a pin/wire/box already carries, and what survives a pop-out's `localStorage` snapshot), `depth` defaults to one hop and clamps to `limits.depth`, and `limits` is partial (`{"fanout": 8}` inherits the rest). The pane sends its whole step list every time and gets a whole graph back — pin ids are minted per call, so ids from two calls cannot be merged; re-deriving is what keeps one graph self-consistent, and it leaves no per-pane trace state on the backend. An unresolvable path **errors** rather than quietly returning a smaller graph. A step's `fanout` (#244 PR4) overrides `limits.fanout` for that step alone and is **not** clamped to it — the "N more…" badge's whole job is to exceed the shared cap for one signal |
