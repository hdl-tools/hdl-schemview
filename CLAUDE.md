...
# deliverable. Everything is --offline, so it runs on an isolated machine:
core/scripts/scale-bench-collect.ps1 [-Full] [-Model <hierarchy.json>] [-SkipCriterion]
    [-SkipReport] [-Out <path>]
# POSIX sibling (what nightly CI runs, with --online since a cold registry cache
# makes the default --offline fail):
bash core/scripts/scale-bench-collect.sh [--full] [--model <hierarchy.json>] [--online] \
    [--bases "golden 665"] [--skip-criterion] [--skip-report] [--out <path>]
# The Rust collector (#240) — same three layers, same default output path, no shell.
# Flags are the .sh's superset; this is the code the packaged app's `--bench` runs:
